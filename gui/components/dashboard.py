...
# Ensure trading_bot is in path for Alpaca tool
...
def render_dashboard():
        ...
        if st.button("Refresh from Alpaca", help="Fetch live data from Alpaca instead of DB"):
            with st.spinner("Fetching..."):
                ...
                if "error" not in portfolio_data:
                    ...
                    # Store in session state to temporarily override db values
    ...
    if not df_history.empty:
        # Streamlit charts expect time to be sorted ascending for plotting
